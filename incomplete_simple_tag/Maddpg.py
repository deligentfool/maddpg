...
class Maddpg(object):
    def __init__(self, name, a_dim, s_dim, agent, sess, hidden_unit=128, tau=0.01, gamma=0.9, a_learning_rate=1e-3, c_learning_rate=2e-3):
        self.sess = sess
        self.tau = tau
        self.gamma = gamma
        self.agent = agent
        ...
        self.other_S = tf.placeholder(tf.float32, [None, s_dim * (self.agent - 1)], 'other_s')
        self.other_S_ = tf.placeholder(tf.float32, [None, s_dim * (self.agent - 1)], 'other_s_')
    ...
    def _build_c(self, s, a, scope, trainable):
        with tf.variable_scope(scope):
            ...
            w1_s = tf.get_variable('w1_s', [self.s_dim*self.agent, n_l1], trainable=trainable)
